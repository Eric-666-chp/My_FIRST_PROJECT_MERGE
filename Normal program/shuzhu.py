...
while game_status == 'yes':
    ...
    def room_2():
        if room_status[2] == 0:
            ...
            choice = ['1']
            answer = ["you open the backpack and found that there was another treasure map inside, and some items:"
                      "flashlight, spatula, lollipop, another treasure map. You have a bag now."]
            global  bag
            bag = ["flashlight","spatula","lollipop", "treasure map"]
            room_status[2] = 1
    ...
    def room_3():
        if room_status[2] == 1:
            ...
            connections = [1, 2]
            directions = ["w", "nw"]
            description = "You sailed to the island, and you saw a huge starfish guarding another treasure map,you need to find a way to get the treasure map it protects before you can go to the next location."
            action = """You have two options:1. Use a spatula to break it up 
                        2. 2. Give a lollipop to the starfish (if you choose to give a lollipop to starfish, it will become your friend and help you) """
            choice = ['1','2']
            answer= [enemy(), "You successfully made friends with Starfish! You got the treasure map he guarded!"]
            room_status[2] = 2
            room_status[3] = 1
            return (roomname, connections, directions, description,action, choice, answer)
        if room_status[2] == 2:
    ...
    def room_4():
        if room_status[3] == 1:
            ...
            connections = [1, 5]
            directions = ["e", "s"]
            description = "According to the instructions of the treasure map, you come to the abandoned factory and you see another treasure map guarded by the yellow square."
            action = """you have three choices:
                        1. Fight with it with a spatula 
                        2. Use a lollipop to buy him off (if you have a lollipop)
            ...
            choice = ['1','2','3']
            answer = [enemy(),"You successfully made friends with yellow square!", "With the help of the starfish, the yellow square will no longer be your enemy"]
            room_status[3] = 2
            room_status[4] = 2
            return (roomname, connections, directions, description,action,choice,answer)
        if room_status[4] == 2:
    ...
    def room_5():
        ...
        action = "you only have one choices: 1.Fight for wealth! ! ! ! ! ! ! !！！！！！！！！！"
        choice = ['1']
        answer = [enemy()]
        return (roomname, connections, directions, description, action, choice, answer)
    ...
    while True:
        ...
        if action == '2':
            if bag[2] != 'lollipop':
                ...
                print()
                game_status = input("Do you want to play againt? (yes or no): ")
                break
        if action == '2':
            bag.pop(2)
        if action == '3':
        ...
        print("you can go: ", roominfo_list[2])
